...
local ContextDownImage = "rbxassetid://97166756"
local ContextUpImage = "rbxassetid://97166444"

local oldTouches = { }

local buttonPositionTable = {	
								[1] = UDim2_new(0, 123, 0, 70), 
								[2] = UDim2_new(0, 30, 0, 60),
								[3] = UDim2_new(0, 180, 0, 160),
								[4] = UDim2_new(0, 85, 0, -25),
								[5] = UDim2_new(0, 185, 0, -25),
								[6] = UDim2_new(0, 185, 0, 260),
								[7] = UDim2_new(0, 216, 0, 65)
							}
...
function createContextActionGui()
	if not buttonScreenGui and isTouchDevice then
		buttonScreenGui = Instance_new("ScreenGui")
		...
		buttonFrame = Instance_new("Frame")
		buttonFrame.BackgroundTransparency = 1
		buttonFrame.Size = UDim2_new(0.3, 0, 0.5, 0)
		buttonFrame.Position = UDim2_new(0.7, 0, 0.5, 0)
...
function setButtonSizeAndPosition(object)
	...
	object.Size = UDim2_new(0, buttonSize, 0, buttonSize)
end

function contextButtonDown(button, inputObject, actionName)
	if inputObject.UserInputType == UserInputType.Touch then
		button.Image = ContextDownImage
		contextActionService:CallFunction(actionName, UserInputState.Begin, inputObject)
	end
end
...
function contextButtonMoved(button, inputObject, actionName)
	if inputObject.UserInputType == UserInputType.Touch then
		button.Image = ContextDownImage
		contextActionService:CallFunction(actionName, UserInputState.Change, inputObject)
	end
end

function contextButtonUp(button, inputObject, actionName)
	button.Image = ContextUpImage
	if inputObject.UserInputType == UserInputType.Touch and inputObject.UserInputState == UserInputState.End then
		contextActionService:CallFunction(actionName, UserInputState.End, inputObject)
	end
end
...
function createNewButton(actionName, functionInfoTable)
	local contextButton = Instance_new("ImageButton")
	contextButton.Name = "ContextActionButton"
	contextButton.BackgroundTransparency = 1
	contextButton.Size = UDim2_new(0, 45, 0, 45)
	contextButton.Active = true
	if isSmallScreenDevice() then contextButton.Size = UDim2_new(0, 35, 0, 35) end
	contextButton.Image = ContextUpImage
	contextButton.Parent = buttonFrame
	local currentButtonTouch = nil
	userInputService.InputEnded:Connect(function(inputObject) oldTouches[inputObject] = nil end)
	contextButton.InputBegan:Connect(function(inputObject)
		if oldTouches[inputObject] then return end
		if inputObject.UserInputState == UserInputState.Begin and currentButtonTouch == nil then
	...
	local actionIcon = Instance_new("ImageLabel")
	actionIcon.Name = "ActionIcon"
	actionIcon.Position = UDim2_new(0.175, 0, 0.175, 0)
	actionIcon.Size = UDim2_new(0.65, 0, 0.65, 0)
	...
	local actionTitle = Instance_new("TextLabel")
	actionTitle.Name = "ActionTitle"
	actionTitle.Size = UDim2_new(1, 0, 1, 0)
	actionTitle.BackgroundTransparency = 1
	actionTitle.Font = Font.SourceSansBold
	actionTitle.TextColor3 = Color3_new(1, 1, 1)
	actionTitle.TextStrokeTransparency = 0
	actionTitle.TextSize = 18
--	actionTitle.FontSize = FontSize.Size18
