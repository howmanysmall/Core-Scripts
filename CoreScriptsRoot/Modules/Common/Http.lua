...
BaseUrl = BaseUrl:gsub("/m.", "/www.")
BaseUrl = BaseUrl:gsub("http://", "https://")

local AssetGameBaseUrl = BaseUrl:gsub("https://www.", "https://assetgame.")
function Http.GetBaseUrl() return BaseUrl end
function Http.GetAssetGameBaseUrl() return AssetGameBaseUrl end
...
function Http.EncodeJSON(jsonTable)
	local success, result = pcall(function() return HttpService:JSONEncode(jsonTable) end)
	if not success then
...
function Http.RbxGetAsync(path, returnRaw)
	local success, result = pcall(function() return game:HttpGetAsync(path) end)
	if not success then
		print("Http.RbxGetAsync() failed because", result, "for path:", path)
		return
	end
	if returnRaw then return result end
	return Http.DecodeJSON(result)
end

function Http.RbxPostAsync(path, returnRaw)
	local success, result = pcall(function() return game:HttpPostAsync(path, '') end)
	if not success then
...
function Http.RbxApiGetAsync(path, throttlePriority, returnRaw)
	...
	local success, result = pcall(function() return HttpRbxApiService:GetAsync(path, throttlePriority) end)
	if not success then
...
function Http.RbxApiPostAsync(path, params, throttlePriority, contentType, returnRaw)
	...
	local success, result = pcall(function() return HttpRbxApiService:PostAsync(path, params, throttlePriority, contentType) end)
	if not success then
